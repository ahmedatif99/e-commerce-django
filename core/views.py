...
def add_to_cart(req, slug):
    ...
    order_item, created = OrderItem.objects.get_or_create(
        item=item, user=req.user, orderd=False)
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            order_item.save()
            messages.warning(req, 'Item quantity updated successfuly')
            return redirect("core:order-summery")
        else:
            order.items.add(order_item)
            messages.warning(req, 'Item added successfuly')
            return redirect("core:order-summery")

    else:
        orderd_date = timezone.now()
        order = Order.objects.create(user=req.user, orderd_date=orderd_date)
        order.items.add(order_item)
        messages.warning(req, 'Item added successfuly')
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            order_item = OrderItem.objects.filter(
                item=item, user=req.user, orderd=False)[0]
            order.items.remove(order_item)
            messages.warning(req, 'Item removed successfuly')
            return redirect("core:product", slug=slug)
        else:
            messages.warning(req, 'This Item is not in your cart. ')
            return redirect("core:product", slug=slug)
    else:
        # add message [the user dose not have an order]
        messages.warning(req, 'You dont have an active order')
        return redirect("core:product", slug=slug)


@login_required
def remove_single_item_from_cart(req, slug):
    item = get_object_or_404(Item, slug=slug)
    order_qs = Order.objects.filter(user=req.user, orderd=False)
    if order_qs.exists():
        order = order_qs[0]
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            if order_item.quantity > 1:
                ...
                order.items.remove(order_item)
            messages.warning(req, 'Item quantity was updated successfuly')
            return redirect("core:order-summery")
        else:
            messages.warning(req, 'This Item is not in your cart. ')
            return redirect("core:order-summery")
...
class CheckoutView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            messages.warning(self.request, "You don't have any active order")
            return redirect("core:order_summery")


class PaymentView(View):
    def get(self, *args, **kwargs):
        # order
        if Order.objects.get(
            user=self.request.user,
            orderd=False
        ):
            if Order.objects.get(
    ...
    def post(self, *args, **kwargs):
        if Order.objects.get(
            ...
            if form.is_valid():
                ...
                csi = userprofile.stripe_customer_id

                if save:
                    if csi != '' and csi is not None:
                ...
                try:
                    ...
                    messages.success(
                        self.request, "Your Order paied done successful!")
                    return redirect("/")
                    ...
                    messages.warning(
                        self.request,
                        "Sth went wrong, You were not charged pleas try again")
                    return redirect("/")
                except Exception:
                    messages.warning(
                        self.request,
...
def get_coupon(req, code):
    try:
        coupon = Coupon.objects.get(code=code)
        return coupon
    except ObjectDoesNotExist:
        messages.warning(req, "This Coupon dose'nt exist")
        return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                order.save()
                messages.success(
                    self.request, "The coupon is applied successfully")
                return redirect("core:checkout")
            except ObjectDoesNotExist:
                messages.warning(
                    self.request, "You don't have any active order")
                return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                refund.save()
                messages.success(
                    self.request, "Your refund request was recived")
                return redirect("core:request-refund")
            except ObjectDoesNotExist:
                messages.warning(
                    self.request, "This order dose'nt exist")
                return redirect("core:request-refund")
